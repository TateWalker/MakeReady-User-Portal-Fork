...
import { normalizeWheel } from "../../lib/utils/normalize-wheel";
import { stopPropagation } from "../../lib/utils/stop-propagation";
...
export interface IScrollView extends INestedChildren<React.ReactNode> {
  ...
  /**
   * Disables a lot of controls this scrollview offers, and hides the custom
   * scrollbar. But makes scrolling exactly how it would show for a overflow:
   * auto.
   */
...
export class ScrollView extends React.Component<IScrollView, IState> {
  ...
  componentDidUpdate() {
    ...
    if (
      ...
      if (this.props.autoScrollDuration === 0) {
        ...
        if (horizontal) {
          this.applyScrollLeft(startValue);
        } else {
          this.applyScrollTop(startValue);
        }
      }
  ...
  private applyScroll(scroll: number) {
    if (this.props.horizontal) {
      this.applyScrollLeft(scroll);
    } else {
      this.applyScrollTop(scroll);
    }
  }

  private applyScrollLeft(scroll: number) {
    if (this.container.current) this.container.current.scrollLeft = scroll;

    if (this.barRef.current) {
      const top = this.container.current?.scrollLeft || 0;
      let barTop = top;
      const metrics = this.getScrollMetrics();
      barTop += (top / metrics.scrollSpace) * metrics.barSpace;
      this.barRef.current.style.left = `${barTop}px`;
    }
  }

  private applyScrollTop(scroll: number) {
    if (this.container.current) this.container.current.scrollTop = scroll;

    if (this.barRef.current) {
      const top = this.container.current?.scrollTop || 0;
      let barTop = top;
      const metrics = this.getScrollMetrics();
      barTop += (top / metrics.scrollSpace) * metrics.barSpace;
      this.barRef.current.style.top = `${barTop}px`;
    }
  }
  ...
  private animateScrollOnce = (startValue: number, scrollTo: number) => {
    ...
    const doAnimate = (t: number) => {
      ...
      this.applyScroll(Math.max(Math.min(scroll || 0, metrics.scrollSpace), 0));
  ...
  handleTouchEnd = (e: TouchEvent) => {
    ...
    if (Math.abs(this.touch.inertia) > 0 && this.container.current) {
      ...
      const animateInertia = () => {
        if (horizontal) {
          this.applyScrollLeft((current.scrollLeft = this.touch.inertia));
        } else this.applyScrollTop(current.scrollTop + this.touch.inertia);

        const metrics = this.getScrollMetrics();

        if (horizontal) {
          this.applyScrollLeft(
            Math.min(current.scrollLeft, metrics.scrollSpace)
          );
        } else {
          this.applyScrollTop(Math.min(current.scrollTop, metrics.scrollSpace));
        }
        ...
        this.touch.inertia *= 0.965;

        if (Math.abs(this.touch.inertia) > 0.5) {
  ...
  handleTouchMove = (e: TouchEvent) => {
    ...
    // Get the touch that started the scrolling
    let touch: Touch | null = null;
    for (let i = 0, iMax = e.touches.length; i < iMax; ++i) {
      touch = e.touches.item(i);
      if (touch) {
    ...
    if (horizontal) {
      this.applyScrollLeft(
        Math.min(e.currentTarget.scrollLeft, metrics.scrollSpace)
      );
    } else {
      this.applyScrollTop(
        Math.min(e.currentTarget.scrollTop, metrics.scrollSpace)
      );
    }
  ...
  handleWheel = (e: WheelEvent) => {
    ...
    if (horizontal) {
      ...
      this.applyScrollLeft(e.currentTarget.scrollLeft + wheel.pixelX);
      this.applyScrollLeft(e.currentTarget.scrollLeft - wheel.pixelY);
    } else this.applyScrollTop(e.currentTarget.scrollTop - wheel.pixelY);
  ...
  handleBarDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const mousemove = (e: MouseEvent) => {
      ...
      if (this.container.current) {
        ...
        if (horizontal) {
          this.applyScrollLeft(
            Math.min(this.container.current.scrollLeft, metrics.scrollSpace)
          );
        } else {
          this.applyScrollTop(
            Math.min(this.container.current.scrollTop, metrics.scrollSpace)
          );
        }
  ...
  render() {
    ...
    if (nativeScroll) {
      return this.renderNativeScroll();
    }
